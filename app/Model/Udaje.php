...
class Udaje extends Table {
  ...
  /** 
   * Vrati pozadovany zaznam kluca alebo null
  ...
  /** 
   * Vrati pozadovany zaznam kluca ako bool
   * @param string $name Nazov kluca
   * @return bool */
  public function getValBool(string $name): bool {
    $p = $this->getByName($name);
    return $p !== null ? (bool)$p->{self::COLUMN_TEXT} : false;
  }
  ...
  public function getUdajInt(string $nazov): int {
    ...
    return $p !== null ? (int)$p->{self::COLUMN_TEXT} : 0;
  }
