 ...
 * Autorizator
 * 
 * Posledna zmena(last change): 02.02.2022
 * 
  ...
  public static function create(Nette\Database\Explorer $database) :Nette\Security\Permission {
    ...
    foreach ($permission as $perm) {
      if ($perm->{self::COLUMN_RESOURCE_ACTIONS} !== NULL) {
        $acl->allow($perm->{self::TABLE_NAME_ROLES}->{self::COLUMN_ROLE}, $perm->{self::TABLE_NAME_RESOURCE}->{self::COLUMN_RESOURCE_NAME}, explode(',', $perm->{self::COLUMN_RESOURCE_ACTIONS}));
      } else {
        $acl->allow($perm->{self::TABLE_NAME_ROLES}->{self::COLUMN_ROLE}, $perm->{self::TABLE_NAME_RESOURCE}->{self::COLUMN_RESOURCE_NAME}, Nette\Security\Permission::ALL);
      }
    }
    return $acl;
  }
