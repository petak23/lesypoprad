...
/**
 * Prezenter pre pristup k api faktúr.
 * Posledna zmena(last change): 05.10.2022
 *
 * Modul: API
 *
...
class FakturyPresenter extends BasePresenter
{
  ...
  {
    $this->sendJson($this->faktury->getItem($id));
  }
  ...
  {
    $this->sendJson($this->faktury->getItemsArray($id));
  }
  ...
  private function _saveDocument(FileUpload $file, int $id_doc, array $data_save): ?array
  {
    ...
    $vysledok = $this->faktury->uloz($data_save, $id_doc);
    if (!empty($vysledok) && isset($file_info['is_image']) && $file_info['is_image']) {
      $this->faktury->repair($vysledok['id'], ['znacka' => '#I-' . $vysledok['id'] . '#']);
    }

    return !empty($vysledok) ? $this->faktury->getDocument($vysledok['id']) : null;
  }
  ...
  private function _uploadThumb(int $id, FileUpload $thumb): String
  {
    if ($id) { // Mazanie sa vykoná len ak je $id > 0
      $pr = $this->faktury->find($id); //Zmazanie starej prílohy
      if ($pr !== FALSE) {
        if (is_file($pr->thumb_file)) {
          unlink($this->wwwDir . "/" . $pr->thumb_file);
        }
  ...
  public function actionDelete(int $id)
  {
    if ($this->getUser()->isLoggedIn() && $this->getUser()->isAllowed($this->name, $this->action)) { //Preventývna kontrola
      $out = ($this->faktury->removeFile($id)) ? ['status' => 200, 'data' => 'OK'] : ['status' => 500, 'data' => null];
    } else {
  ...
  /** Vymazanie viacerých položiek z DB */
  public function actionDeleteMore()
  {
    if ($this->getUser()->isLoggedIn() && $this->getUser()->isAllowed($this->name, $this->action)) { //Preventývna kontrola
      ...
      foreach ($values['to_del'] as $k => $v) {
        if (!$this->faktury->removeFile($v)) $o = false;
      }
  ...
  /** 
   * Oprava položky v DB 
   * @param int $id Id_hlavne_menu, ku ktorému ukladám položku */
  public function actionUpdate(int $id): void
  {
    /* from POST: */
    $values = json_decode(file_get_contents("php://input"), true); // @help 1.)

    $this->faktury->uloz($values, $id);
  ...
  public function actionChangeperpage(): void
  {
    ...
    $out = $this->udaje->editKey('faktury_per_page', $values['items_per_page']);
  ...
  {
    $this->sendJson((int)$this->udaje->getValByName('faktury_per_page'));
  }
}
