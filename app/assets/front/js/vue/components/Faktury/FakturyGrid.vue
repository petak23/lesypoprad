...
export default {
  ...
  data() {
    return {
      fields: [
          ...
          {
            key: 'datum_vystavenia',
            label: 'Dátum uzatvorenia zmluvy',
            tdClass: "position-relative"
          },
          {
            key: 'datum_ukoncenia',
  ...
  methods: {
    ...
    openmodal(index) {
      ...
      this.$bvModal.show("modal-multi-faktury");
    },
    closeme: function () {
      this.$bvModal.hide("modal-multi-faktury");
    },
    imgUrl() {
    ...
    loadItems() { // Načítanie údajov priamo z DB
      ...
      axios
        ...
        .catch((error) => {
          this.error_msg = 'Nepodarilo sa načítať údaje do tabuľky. <br/>Možná príčina: ' + error
          this.loading = 2
  ...
  created() {
    if (this.editEnabled || this.delEnabled) // Ak ja povolená editácia alebo mazanie 
    { 
      /*this.fields.unshift( // Pridanie označovacieho stĺpca na začiatok
        {
          key: 'selected',
          label: 'Označ',
      ...
      this.fields.push( // Pridanie stĺpca akcie na koniec
        {
          key: 'action',
          label: 'Akcie',
        }
      ...
      if (this.zmluvy == "0") {
        const inn = ['nazov', 'datum_ukoncenia'];
        this.fields = this.fields.filter(function(e) { return inn.indexOf(e.key) == -1})
      }
    }
    // Načítanie údajov priamo z DB
    this.loadItems()
    
    this.$root.$on('faktury_add', data => {
			this.items.push(...data)
      this.items_count()
		})

    this.$root.$on('faktury_delete', this.deleteMore)
  },
};
</script>
...
<template>
  <div>
    ...
    <!-- select-mode="multi"
      ...
      <!--template #head(selected) v-if="editEnabled || delEnabled">
      ...
      <template #cell(selected)="{ rowSelected }" v-if="editEnabled || delEnabled">
      ...
      <template #cell(cislo)="data">
        <text-cell
          :value="data.item.cislo"
          :apiLink="basePath + baseApiPath + 'update/'"
          colName="cislo"
          :id="data.item.id"
          :editEnabled="false && editEnabled"
        ></text-cell>
      </template>
      ...
      <template #cell(nazov)="data">
        <text-cell
          :value="data.item.nazov"
          :apiLink="basePath + baseApiPath + 'update/'"
          colName="nazov"
          :id="data.item.id"
          :editEnabled="false && editEnabled"
      ...
      <template #cell(predmet)="data">
        <text-cell
          :value="data.item.predmet"
          :apiLink="basePath + baseApiPath + 'update/'"
          colName="predmet"
          :id="data.item.id"
          :editEnabled="false && editEnabled"
      ...
      <template #cell(datum_vystavenia)="data">
        <div>{{ data.item.datum_vystavenia| formatDate}}</div>
      </template>
      <template #cell(datum_ukoncenia)="data">
        <div>{{ data.item.datum_ukoncenia| formatDate}}</div>
      </template>
      ...
      <template #cell(action)="data" v-if="editEnabled || delEnabled">
        <!--button type="button" class="btn btn-info btn-sm" title="Edit">
          <i class="fa-solid fa-pen"></i>
        </button-->
        
        <a :href="'?viewFaktury-id=' + data.item.id + '&do=viewFaktury-edit'" 
          v-if="editEnabled" 
          title="edit"
    ...
    <b-modal
      id="modal-multi-faktury"
      centered
      size="xl"
      ok-only
      hide-header
      hide-footer
      dialog-class="faktury-dialog"
    >
