...
//    }
    ...
    //login ok
    $this->database->table('user_prihlasenie')->insert(array(
      'id_user_profiles'  =>$prof1->id,
      'prihlasenie_datum' => StrFTime("%Y-%m-%d %H:%M:%S", Time()),
    ));
//		unset($row->password);
		$uzivatel = $row->toArray(); //data z tabulky users
		$prof = $prof1->toArray();
		unset($prof['id'], $prof['id_users']);unset($uzivatel['id'], $uzivatel["password"]);
		$uzivatel = array_merge($prof, $uzivatel); // Zlucenie users a user_profiles
    $nastav = array(
			'pocet_pr'				=> $prof1->pocet_pr+1,
			'prihlas_teraz'		=> StrFTime("%Y-%m-%d %H:%M:%S", Time()),
			'prihlas_predtym'	=> $prof1->prihlas_teraz,
		);
    //Docasne riadky
    if (isset($row->created) && $prof1->created == NULL) $nastav['created'] = $row->created;
    if (isset($row->modified) && $prof1->modified == NULL) $nastav['modified'] = $row->modified;
